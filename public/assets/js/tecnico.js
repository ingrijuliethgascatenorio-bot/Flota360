...
function _alertaMeta(tipo) {
  const map = {
    ...
    documento_30dias:      { color:'yellow', bg:'#fef9e7', border:'#f59e0b', label:'Doc. por vencer',   emoji:'<i class="fa-solid fa-file-lines"></i>' },
    documento_15dias:      { color:'orange', bg:'#fff4e6', border:'#f97316', label:'Doc. urgente',      emoji:'<i class="fa-solid fa-triangle-exclamation"></i>'  },
    documento_7dias:       { color:'red',    bg:'#fff0f0', border:'#ef4444', label:'¡Doc. crítico!',    emoji:'<i class="fa-solid fa-circle-exclamation"></i>' },
    documento_vencido:     { color:'red',    bg:'#fff0f0', border:'#ef4444', label:'Doc. vencido',      emoji:'<i class="fa-solid fa-circle-xmark"></i>' },
  };
...
function _renderAlertaDash(a) {
  ...
  return `
  <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px 10px; border-bottom: 1px solid var(--slate-100); cursor: pointer; transition: background 0.2s;" onmouseover="this.style.background='var(--slate-50)'" onmouseout="this.style.background='transparent'" onclick="${a.ordenId ? `abrirDetalleOrden(${a.ordenId})` : ''}">
    <div style="display: flex; align-items: center; gap: 10px; flex: 1; min-width: 0;">
      <div style="color: ${m.border}; font-size: 16px; display: flex; align-items: center; justify-content: center; width: 20px; flex-shrink: 0;">
        ${m.emoji}
      </div>
      <div style="min-width: 0;">
        <div class="ad-short" style="font-weight: 600; font-size: 13px; color: var(--navy); line-height: 1.3;">${shortDesc}</div>
        <div class="ad-full" style="font-weight: 600; font-size: 14px; color: var(--navy); line-height: 1.3;">${fullMsg}</div>
        <div style="font-size: 12px; color: var(--text-lt); margin-top: 2px;">🚗 ${a.placa || 'N/D'}</div>
      </div>
    </div>
    <span class="ad-badge" style="font-size: 11px; font-weight: 700; color: ${m.border}; background: ${m.bg}; padding: 3px 8px; border-radius: 99px; white-space: nowrap; margin-left: 8px;">${m.label}</span>
  </div>`;
}
...
async function cargarAlertasDash() {
  ...
  try {
    ...
    const urgentes = todas.slice(0, 3);
    ...
    // Alimentar panel campana desktop (FC_ADMIN_PANEL) con las mismas alertas
    if (typeof FC_ADMIN_PANEL !== 'undefined' && typeof FC_ADMIN_PANEL.addItems === 'function') {
      const tipoMap = {
        orden_nueva:           { icon: '<i class="fa-solid fa-clipboard-list"></i>', type: 'info'    },
        mantenimiento_proximo: { icon: '<i class="fa-solid fa-wrench"></i>', type: 'warning' },
        mantenimiento_vencido: { icon: '<i class="fa-solid fa-siren-on"></i>', type: 'danger'  },
        documento_30dias:      { icon: '<i class="fa-solid fa-file-lines"></i>', type: 'info'    },
        documento_15dias:      { icon: '<i class="fa-solid fa-triangle-exclamation"></i>', type: 'warning' },
        documento_7dias:       { icon: '<i class="fa-solid fa-circle-exclamation"></i>', type: 'danger'  },
        documento_vencido:     { icon: '<i class="fa-solid fa-circle-xmark"></i>', type: 'danger'  },
      };
      const notifItems = todas.map((a, idx) => {
        const cfg = tipoMap[a.tipoAlerta] || { icon: '<i class="fa-solid fa-bell"></i>', type: 'info' };
        return {
          id: a.ordenId ? `ord-${a.ordenId}` : `alerta-${idx}`,
          icon: cfg.icon,
          type: cfg.type,
          ...
          unread: true,
          _date: a.generadaEn,
          onClick: () => showPage('alertas', 'Alertas'),
        };
      });
